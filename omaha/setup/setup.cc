...
// Returns the pids of all other DentalWingsUpdate.exe processes with the specified
...
HRESULT Setup::WaitForOtherInstancesToExit(const Pids& pids,
  ...
  if (!handles.empty()) {
    ...
    if (WAIT_FAILED == res) {
      ...
      OPT_LOG(LEVEL_ERROR, (_T("[Other DentalWingsUpdate.exe instances failed to ")

...
class Setup {
 ...
 private:
  ...
  // Returns how long to wait before terminating DentalWingsUpdate.exe forcefully.
  ...
  // Waits for other instances of DentalWingsUpdate.exe to exit.
  HRESULT WaitForOtherInstancesToExit(const Pids& pids,
                                      int wait_time_before_kill_ms);

  // Gets the list of all the DentalWingsUpdate.exe processes to wait for.
  HRESULT GetPidsToWaitFor(Pids* pids) const;

  // Gets a list of DentalWingsUpdate.exe processes for user or machine that are

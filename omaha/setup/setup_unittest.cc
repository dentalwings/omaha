...
class SetupTest : public testing::Test {
 protected:
  ...
  // Returns the path to the long-running DentalWingsUpdate.exe.
  ...
  explicit SetupTest(bool is_machine)
      : is_machine_(is_machine),
        ...
        not_listening_exe_opposite_path_(!is_machine ?
                                         not_listening_machine_exe_path_ :
                                         not_listening_user_exe_path_) {
    omaha_exe_path_ = ConcatenatePath(omaha_path_, _T("DentalWingsUpdate.exe"));
  }
  ...
  // Launches an instance of DentalWingsUpdate.exe that doesn't exit.
